...
const renderRestaurants = userCoordinates => {
  const sorted = [...restaurants].sort((first, second) => {
    const firstDistance = distance(first.location.coordinates, userCoordinates);
    const secondDistance = distance(
  ...
  sorted.forEach(restaurant => {
    ...
    addressCell.textContent = `${restaurant.address}, ${restaurant.city}`;

    row.append(nameCell, addressCell);
    table.append(row);
  });
};
...
navigator.geolocation.getCurrentPosition(
  position => {
    renderRestaurants([position.coords.longitude, position.coords.latitude]);
  },
  () => {
    renderRestaurants([24.9384, 60.1699]);
  }
);
