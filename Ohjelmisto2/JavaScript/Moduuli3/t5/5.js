...
const osio = document.querySelector("section");
picArray.forEach((kuva) => {
  const artikkeli = document.createElement("article");
  artikkeli.className = "card";

  const otsikko = document.createElement("h2");
  otsikko.textContent = kuva.title;

  const kuvio = document.createElement("figure");
  const img = document.createElement("img");
  img.src = kuva.image.medium;
  img.alt = kuva.title;
  const kuvateksti = document.createElement("figcaption");
  kuvateksti.textContent = kuva.caption;
  kuvio.appendChild(img);
  kuvio.appendChild(kuvateksti);

  const kuvaus = document.createElement("p");
  kuvaus.textContent = kuva.description;

  artikkeli.appendChild(otsikko);
  artikkeli.appendChild(kuvio);
  artikkeli.appendChild(kuvaus);

  osio.appendChild(artikkeli);
});
